...
function showProximityPanel(hub) {
  ...
  if (isActive) {
    ...
    pStatus.innerHTML = `<span style="color:#3fb950">🟢 Đang có meeting · ${n} người</span>`;
  } else {
    pStatus.innerHTML = `<span style="color:#8b949e">⚪ Chưa có meeting</span>`;
  }
...
document.getElementById("create-meet-btn").addEventListener("click", () => {
  ...
  const t = setInterval(() => {
    cd--;
    if (cd <= 0) { clearInterval(t); window.open(link,"_blank"); note.textContent = "Đã mở tab mới!"; }
    else note.textContent = `Link tự mở sau ${cd}s...`;
  }, 1000);
});

document.getElementById("copy-link-btn").addEventListener("click", () => {
  if (!currentHub) return;
  let link = STATE.meetLinks[currentHub.id]?.link;
  if (!link) {
    link = generateMeetLink();
    STATE.meetLinks[currentHub.id] = { active:true, link, attendees:1, host:STATE.playerName, startedAt:Date.now() };
    saveMeetLinks();
    const lb = document.getElementById("meet-link-box");
    lb.classList.remove("hidden");
